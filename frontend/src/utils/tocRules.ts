export interface TocRule {
  enable: boolean;
  example: string;
  id: number;
  name: string;
  rule: string;
  serialNumber: number;
  type?: 'regex' | 'wordCount';
  wordCountOptions?: {
    wordsPerChapter: number;
    showFirstSentence: boolean;
  };
}

export interface CustomRule {
  id: string;
  name: string;
  rule: string;
  example: string;
  enabled: boolean;
  custom: true;
}

// 默认的目录规则，直接写入代码中
export const DEFAULT_TOC_RULES: TocRule[] = [
  {
    enable: true,
    example: "第一章 假装第一章前面有空白但我不要",
    id: -1,
    name: "目录(去空白)",
    rule: "(?<=[　\\s])(?:序章|楔子|正文(?!完|结)|终章|后记|尾声|番外|第\\s{0,4}[\\d〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+?\\s{0,4}(?:章|节(?!课)|卷|集(?![合和])))[\\s\\S]{0,30}$",
    serialNumber: 0
  },
  {
    enable: true,
    example: "第一章 标准的粤语就是这样",
    id: -2,
    name: "目录",
    rule: "^[ 　\\t]{0,4}(?:序章|楔子|正文(?!完|结)|终章|后记|尾声|番外|第\\s{0,4}[\\d〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+?\\s{0,4}(?:章|节(?!课)|卷|集(?![合和])|部(?![分赛游])|篇(?!张)))[\\s\\S]{0,30}$",
    serialNumber: 1
  },
  {
    enable: false,
    example: "简介 老夫诸葛村夫",
    id: -3,
    name: "目录(匹配简介)",
    rule: "(?<=[　\\s])(?:(?:内容|文章)?简介|文案|前言|序章|楔子|正文(?!完|结)|终章|后记|尾声|番外|第\\s{0,4}[\\d〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+?\\s{0,4}(?:章|节(?!课)|卷|集(?![合和])|部(?![分赛游])|回(?![合来事去])|场(?![和合比电是])|篇(?!张)))[\\s\\S]{0,30}$",
    serialNumber: 2
  },
  {
    enable: false,
    example: "第一章 比上面只多了回和话",
    id: -4,
    name: "目录(古典、轻小说备用)",
    rule: "^[ 　\\t]{0,4}(?:序章|楔子|正文(?!完|结)|终章|后记|尾声|番外|第\\s{0,4}[\\d〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+?\\s{0,4}(?:章|节(?!课)|卷|集(?![合和])|部(?![分赛游])|回(?![合来事去])|场(?![和合比电是])|话|篇(?!张)))[\\s\\S]{0,30}$",
    serialNumber: 3
  },
  {
    enable: false,
    example: "12",
    id: -5,
    name: "数字(纯数字标题)",
    rule: "(?<=[　\\s])\\d+\\.?[ 　\\t]{0,4}$",
    serialNumber: 4
  },
  {
    enable: false,
    example: "一百七十",
    id: -6,
    name: "大写数字(纯数字标题)",
    rule: "(?<=[　\\s])[零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]{1,12}[ 　\\t]{0,4}$",
    serialNumber: 5
  },
  {
    enable: false,
    example: "12\n一百七十",
    id: -7,
    name: "数字混合(纯数字标题)",
    rule: "(?<=[　\\s])[零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟\\d]{1,12}[ 　\\t]{0,4}$",
    serialNumber: 6
  },
  {
    enable: true,
    example: "1、这个就是标题",
    id: -8,
    name: "数字 分隔符 标题名称",
    rule: "^[ 　\\t]{0,4}\\d{1,5}[:：,.， 、_—\\-].{1,30}$",
    serialNumber: 7
  },
  {
    enable: true,
    example: "一、只有前面的数字有差别\n二十四章 我瞎编的标题",
    id: -9,
    name: "大写数字 分隔符 标题名称",
    rule: "^[ 　\\t]{0,4}(?:序章|楔子|正文(?!完|结)|终章|后记|尾声|番外|[零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]{1,8}章?)[ 、_—\\-].{1,30}$",
    serialNumber: 8
  },
  {
    enable: false,
    example: "1、人参公鸡\n二百二十章 boy next door",
    id: -10,
    name: "数字混合 分隔符 标题名称",
    rule: "^[ 　\\t]{0,4}(?:序章|楔子|正文(?!完|结)|终章|后记|尾声|番外|[零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]{1,8}章?[ 、_—\\-]|\\d{1,5}章?[:：,.， 、_—\\-]).{0,30}$",
    serialNumber: 9
  },
  {
    enable: true,
    example: "正文 我奶常山赵子龙",
    id: -11,
    name: "正文 标题/序号",
    rule: "^[ 　\\t]{0,4}正文[ 　]{1,4}.{0,20}$",
    serialNumber: 10
  },
  {
    enable: true,
    example: "Chapter 1 MyGrandmaIsNB",
    id: -12,
    name: "Chapter/Section/Part/Episode 序号 标题",
    rule: "^[ 　\\t]{0,4}(?:[Cc]hapter|[Ss]ection|[Pp]art|ＰＡＲＴ|[Nn][oO][.、]|[Ee]pisode|(?:内容|文章)?简介|文案|前言|序章|楔子|正文(?!完|结)|终章|后记|尾声|番外)\\s{0,4}\\d{1,4}.{0,30}$",
    serialNumber: 11
  },
  {
    enable: false,
    example: "Chapter 1 MyGrandmaIsNB",
    id: -13,
    name: "Chapter(去简介)",
    rule: "^[ 　\\t]{0,4}(?:[Cc]hapter|[Ss]ection]|[Pp]art|ＰＡＲＴ|[Nn][Oo]\\.|[Ee]pisode)\\s{0,4}\\d{1,4}.{0,30}$",
    serialNumber: 12
  },
  {
    enable: true,
    example: "【第一章 后面的符号可以没有",
    id: -14,
    name: "特殊符号 序号 标题",
    rule: "(?<=[\\s　])[【〔〖「『〈［\\[](?:第|[Cc]hapter)[\\d零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]{1,10}[章节].{0,20}$",
    serialNumber: 13
  },
  {
    enable: false,
    example: "『加个直角引号更专业』\n(11)我奶常山赵子聋",
    id: -15,
    name: "特殊符号 标题(成对)",
    rule: "(?<=[\\s　]{0,4})(?:[\\[〈「『〖〔《（【\\(].{1,30}[\\)】）》〕〗』」〉\\]]?|(?:内容|文章)?简介|文案|前言|序章|楔子|正文(?!完|结)|终章|后记|尾声|番外)[ 　]{0,4}$",
    serialNumber: 14
  },
  {
    enable: true,
    example: "☆、晋江作者最喜欢的格式",
    id: -16,
    name: "特殊符号 标题(单个)",
    rule: "(?<=[\\s　]{0,4})(?:[☆★✦✧].{1,30}|(?:内容|文章)?简介|文案|前言|序章|楔子|正文(?!完|结)|终章|后记|尾声|番外)[ 　]{0,4}$",
    serialNumber: 15
  },
  {
    enable: true,
    example: "卷五 开源盛世",
    id: -17,
    name: "章/卷 序号 标题",
    rule: "^[ \\t　]{0,4}(?:(?:内容|文章)?简介|文案|前言|序章|楔子|正文(?!完|结)|终章|后记|尾声|番外|[卷章][\\d零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]{1,8})[ 　]{0,4}.{0,30}$",
    serialNumber: 16
  },
  {
    enable: false,
    example: "20字以内顶格写的都是标题",
    id: -18,
    name: "顶格标题",
    rule: "^\\S.{1,20}$",
    serialNumber: 17
  },
  {
    enable: false,
    example: "第一章 真正的标题\n第一章 这个不要",
    id: -19,
    name: "双标题(前向)",
    rule: "^[ \\t　]{0,4}第[\\d零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+章.{0,30}$[\\s　]{0,8}第[\\d零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+章.{0,30}$",
    serialNumber: 18
  },
  {
    enable: false,
    example: "第一章 这个标题不要\n第一章真正的标题",
    id: -20,
    name: "双标题(后向)",
    rule: "^[ \\t　]{0,4}第[\\d零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+章.{0,30}$[\\s　]{0,8}第[\\d零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+章.{0,30}$",
    serialNumber: 19
  },
  {
    enable: true,
    example: "标题后面数字有括号(12)",
    id: -21,
    name: "书名 括号 序号",
    rule: "^[一-龥]{1,20}[ 　\\t]{0,4}[(（][\\d〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]{1,8}[)）][ 　\\t]{0,4}$",
    serialNumber: 20
  },
  {
    enable: true,
    example: "标题后面数字没有括号124",
    id: -22,
    name: "书名 序号",
    rule: "^[一-龥]{1,20}[ 　\\t]{0,4}[\\d〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]{1,8}[ 　\\t]{0,4}$",
    serialNumber: 21
  },
  {
    enable: false,
    example: "===起这种标题干什么===",
    id: -23,
    name: "特定字符 标题 特定符号",
    rule: "(?<=={3,6}).{1,40}?(?==)",
    serialNumber: 22
  },
  {
    enable: true,
    example: "分节|分页|分段阅读\n第一页",
    id: -24,
    name: "字数分割 分节阅读",
    rule: "(?<=[ 　\\t]{0,4})(?:.{0,15}分[页节章段]阅读[-_ ]|第\\s{0,4}[\\d零一二两三四五六七八九十百千万]{1,6}\\s{0,4}[页节]).{0,30}$",
    serialNumber: 23
  },
  {
    enable: false,
    example: "第一章 0-15\n第1章 开始的故事",
    id: -27,
    name: "简化章节匹配",
    rule: "^\\s*第\\s*[\\d〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+\\s*章",
    serialNumber: 26
  },
  {
    enable: false,
    example: "出事（一）车祸，这种有（序号）的",
    id: -28,
    name: "（序号）",
    rule: ".{0,20}（[零一二三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+）.{0,30}$",
    serialNumber: 27
  },
  {
    enable: false,
    example: "（一）第一句话内容...\n（二）另一句话内容...",
    id: -26,
    name: "按字数分割",
    rule: "WORD_COUNT_SPLIT",
    type: "wordCount",
    wordCountOptions: {
      wordsPerChapter: 10000,
      showFirstSentence: true
    },
    serialNumber: 25
  },
  {
    enable: true,
    example: "激进规则,适配更多非常用格式",
    id: -25,
    name: "通用规则",
    rule: "^[ 　\\t]{0,4}(?:序章|楔子|正文|终章|后记|尾声|番外|第\\s{0,4}[\\d〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+?\\s{0,4}(?:章|节|卷|集|部|篇)|[卷章]\\s*[\\d〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+|番外[\\d〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟]+|[\\d〇零一二两三四五六七八九十百千万壹贰叁肆伍陆柒捌玖拾佰仟a-zA-Z]+?[、. 　])[\\s\\S]{0,40}$",
    serialNumber: 24
  }
];

// 默认启用的规则ID列表（单选模式，默认使用通用规则）
export const DEFAULT_ENABLED_RULE_IDS = [
  -25,  // 通用规则（覆盖面最广，最稳定）
];